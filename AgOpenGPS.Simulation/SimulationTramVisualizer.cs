...
{
    public class SimulationTramVisualizer
    {
        ...
        public SimulationTramVisualizer(IConfiguration configuration)
        {

            tramWidth = configuration.Get<double>("setTram_tramWidth");
            halfWheelTrack = configuration.Get<double>("setVehicle_trackWidth") * 0.5;

            isOuter = ((int)(tramWidth / mf.tool.width + 0.5)) % 2 == 0;

            passes = configuration.Get<int>("setTram_passes");
            displayMode = 0; // You can set displayMode to a specific value or use configuration.Get<int>("displayMode") if it exists.
